...
def get_snowflake_conn():
    """Connects using Key Pair (Local) OR Password (GitHub)"""
    # 1. Try Local Key
...
def load_to_snowflake(data):
    if not data: raise ValueError("No data received")
    conn = get_snowflake_conn()
    cursor = conn.cursor()
    try:
        cursor.execute(f"CREATE TABLE IF NOT EXISTS {SCHEMA}.{TABLE} (record_id VARCHAR DEFAULT UUID_STRING(), ingested_at TIMESTAMP_NTZ DEFAULT CURRENT_TIMESTAMP(), json_data VARIANT)")
        logger.info(f"Inserting {len(data)} records...")
        records = [(json.dumps(r),) for r in data]
        cursor.executemany(f"INSERT INTO {SCHEMA}.{TABLE} (json_data) SELECT PARSE_JSON($1) FROM VALUES (%s)", records)
        conn.commit()
...
if __name__ == "__main__":
    print("DEBUG: STARTING NEW PIPELINE SCRIPT")
    try:
        load_to_snowflake(fetch_crypto_data())
